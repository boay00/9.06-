...
uploader = st.file_uploader('Upload Files')

st.header(':eggplant: Date')
time = st.date_input('Your Favourite Date')
time_2 = st.date_input('Your Least Favourite Date')

slider_pointless = st.slider("Pick a magic number dont ask why", 0, 100)

if slider_pointless > 50:
     st.text('big one!')
elif slider_pointless < 50:
     st.text('bit diddy that')
else:
     st.text('nice')
